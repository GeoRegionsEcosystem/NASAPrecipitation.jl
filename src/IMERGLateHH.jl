...
function download(
	...
	@info "$(now()) - NASAPrecipitation.jl - Downloading Late IMERG half-hourly data for the $(geo.name) GeoRegion from $(ymd2str(npd.dtbeg)) to $(ymd2str(npd.dtend))"
	...
	for dt in npd.dtbeg : Day(1) : npd.dtend
		...
		@info "$(now()) - NASAPrecipitation.jl - Downloading Late IMERG half-hourly data for the $(geo.name) GeoRegion from the NASA Earthdata servers using OPeNDAP protocols for $(ymd2str(dt)) ..."
...
function save(
	...
	@info "$(now()) - NASAPrecipitation.jl - Saving Late IMERG raw half-hourly data in the $(geo.name) GeoRegion for $(ymd2str(dt))"
